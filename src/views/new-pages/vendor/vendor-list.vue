            ...
            <vs-table search max-items="5" pagination :data="userList" class="table-dark-inverted" stripe>
              ...
              <template slot-scope="{data}">
                <vs-tr :key="i" v-for="(tr, i) in data">
                  ...
                  <vs-td>
                    <vs-button :to="{name: 'vendor-view', params: { id: data[i]._id }}" color="primary" icon="create" type="border"></vs-button>
                  </vs-td>
                </vs-tr>
              </template>
            </vs-table>
...
export default {
  ...
  async created () {
    this.loadData()
    this.$vs.loading()
    this.sockets.subscribe('updateVendor', () => {
