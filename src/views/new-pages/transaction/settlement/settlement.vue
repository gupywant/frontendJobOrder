...
<template>
  <div id="transaction-settlement-create">
    <div class="vx-row">
                  ...
                  <vs-table :data="registeredService" class="table-dark-inverted" stripe>
                    ...
                    <template slot-scope="{data}">
                      <vs-tr :key="i" v-for="(tr, i) in data">
                        ...
                        <vs-td>
                          <vs-input style="width: 50px" :disabled="serviceDisabled" v-validate="'numeric'" v-model="qty[j-1][i]" type="text" placeholder="Quantity" />
                        </vs-td>
                        <vs-td>
                          <vs-input :disabled="serviceDisabled" v-validate="'numeric'" v-model="planned_amount[j-1][i]" type="text" placeholder="Planned Amount" />
                        </vs-td>
                        <vs-td>
                          <vs-input style="width: 50px" v-validate="'numeric'" v-model="settlementQty[j-1][i]" type="text" placeholder="Quantity" />
                        </vs-td>
                        <vs-td>
                          <vs-input v-validate="'numeric'" v-model="settlementPlanned_amount[j-1][i]" type="text" placeholder="Planned Amount" />
                        </vs-td>
                      </vs-tr>
                    </template>
                  </vs-table>
  ...
  methods: {
    add () {
      ...
      try {
        this.vendor.forEach((item, i) => {
          ...
          this.settlementQty[i].forEach((qty) => {
            if (qty === null) throw err
            if (qty === '') throw err
          })
          this.settlementPlanned_amount[i].forEach((amount) => {
    ...
    saveTransaction () {
      ...
      const payload = {
        ...
        qty: this.settlementQty,
        planned_amount: this.settlementPlanned_amount 
      }
      this.$vs.loading()
