                ...
                <vs-table search max-items="5" :data="availServiceList" class="table-dark-inverted" stripe>
                  ...
                  <template slot-scope="{data}">
                    <vs-tr :key="i" v-for="(tr, i) in data">
                      ...
                      <vs-td>
                        <vs-input style="width: 50px;" disabled v-validate="'required|numeric'" v-model="data[i].qty" type="text" placeholder="Planned Amount" />
                      </vs-td>
                      <vs-td>
                        <vs-input v-validate="'required|numeric'" disabled v-model="data[i].planned_amount" type="text" placeholder="Planned Amount" />
                      </vs-td>
                      <vs-td>
                        <vs-input style="width: 50px;" v-validate="'required|numeric'" v-model="settlement[i].qty" type="text" placeholder="Planned Amount" />
                      </vs-td>
                      <vs-td>
                        <vs-input v-validate="'required|numeric'" v-model="settlement[i].settlement_amount" type="text" placeholder="Planned Amount" />
                      </vs-td>
                    </vs-tr>
                  </template>
                </vs-table>
    ...
    retrieveTransactionDetail () {
      ...
      this.$store.dispatch('retrieveTransactionDetail', id)
        .then(async (response) => {
          ...
          await response.data.data.map((item) => {
            this.settlement.push({id_service: item.service_id, qty: 0, settlement_amount: 0})
          })
