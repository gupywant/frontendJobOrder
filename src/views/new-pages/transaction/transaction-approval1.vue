...
<template>
  <div id="transaction-approval1">
    <div class="vx-row">
      <!-- CARD 9: DISPATCHED ORDERS -->
      <div class="vx-col w-full">
        <vx-card title="Job Order Approval 1">
...
export default {
  ...
  async created () {
    this.$vs.loading()
    await this.loadData()
    this.sockets.subscribe('updateApproval1', () => {
  ...
  methods: {
    ...
    async loadData () {
      await this.retrieveApprovalList()
    },
    async retrieveApprovalList () {
      ...
      await this.$store.dispatch('retrieveApprovalList', approval)
        .then(async (response) => {
          console.log('coba')
          this.approvalList = await response.data.data
