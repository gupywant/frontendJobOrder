...
<template>
  <div id="transaction-cancel">
    <div class="vx-row">
      <!-- CARD 9: DISPATCHED ORDERS -->
      <div class="vx-col w-full">
        <vx-card title="Job Order Cancelation">
...
export default {
  ...
  methods: {
    handleSelected (tr) {
      this.$router.push({name: 'cancel-view', params: { id: tr._id }})
    },
    codeGenerator (code) {
      const max = 5
      let out = ''
    ...
    async retrieveApprovalList () {
      await this.$store.dispatch('retrieveCancelList')
        .then(async (response) => {
