...
<template>
  <div id="invoice-cacncelation">
    <div class="vx-row">
      <!-- CARD 9: DISPATCHED ORDERS -->
      <div class="vx-col w-full">
        <vx-card title="Invoice cancelation">
  ...
  methods: {
    ...
    confirmCancelInvoice () {
      this.$vs.dialog({
        ...
        color: 'danger',
        title: 'Approving',
        text: 'Continue to cancel this invoice?',
        accept: this.cancelInvoice,
        acceptText: 'Continue'
      })
    },
    cancelInvoice () {
      this.$store.dispatch('cancelInvoice', this.id)
        .then(async () => {
    ...
    showDeleteSuccess () {
      this.$vs.notify({
        ...
        text: 'The selected invoice was successfully canceled'
      })
    },
    codeGenerator (code) {
      const max = 5
