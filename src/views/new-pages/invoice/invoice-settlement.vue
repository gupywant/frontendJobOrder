  ...
  <div id="invoice-cacncelation">
    <div class="vx-row">
      ...
      <div class="vx-col w-full">
        <vx-card title="Invoice Settlement">
          ...
          <div slot="no-body" class="mt-4">
            <vs-table search v-model="selected" @selected="handleSelected" max-items="5" pagination :data="invoiceList" class="table-dark-inverted" stripe>
              ...
              <template slot-scope="{data}">
                <vs-tr :data="i + 1" :key="i" v-for="(tr, i) in data">
...
export default {
  ...
  methods: {
    async handleSelected (i) {
      this.id = await this.invoiceList[i - 1]
      this.$router.push({name: 'invoice-settle', params: { id: this.id._id }})
    },
    codeGenerator (code) {
      const max = 5
      let out = ''
